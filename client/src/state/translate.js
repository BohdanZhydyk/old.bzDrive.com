...
export const translate = (lang = 'en', txt)=>{
  switch(txt){
    ...
    case "role":
      ...
      if(lang === 'ua'){ return `роль` }
      if(lang === 'pl'){ return `rola` }
      if(lang === 'ru'){ return `роль` }
      break
    case "sex":
      if(lang === 'en'){ return `sex` }
      if(lang === 'ua'){ return `стать` }
      if(lang === 'pl'){ return `plec` }
      if(lang === 'ru'){ return `стать` }
      break
    case "language":
      if(lang === 'en'){ return `language` }
      if(lang === 'ua'){ return `мова` }
      if(lang === 'pl'){ return `język` }
    ...
    case "MonthNames":
      ...
      break
    case "SettingsHintTitle":
      if(lang === 'en'){ return `Seller information` }
      if(lang === 'ua'){ return `Інформація про продавця` }
      if(lang === 'pl'){ return `Informacja sprzedawcy` }
      if(lang === 'ru'){ return `Информация о продавце` }
      break
    case "SettingsHintTxt":
      if(lang === 'en'){ return [
        `Here you can edit the avatar and basic information of the seller for the documents generated by the program.`,
        `Just enter the required data and click the "Save" button.`
      ]}
      if(lang === 'ua'){ return [
        `Тут можна редагувати аватар та основну інформацію продавця для згенерованих програмою документів.`,
        `Просто внеси потрібні дані і натисни кнопку "Зберегти".`
      ]}
      if(lang === 'pl'){ return [
        `Tutaj możesz edytować awatar i podstawowe informacje o sprzedawcy dla dokumentów generowanych przez program.`,
        `Wystarczy wpisać wymagane dane i kliknąć przycisk "Zapisz".`
      ]}
      if(lang === 'ru'){ return [
        `Здесь можно редактировать аватар и основную информацию продавца для сгенерированных программой документов.`,
        `Просто внесите нужные данные и нажми кнопку "Сохранить".`
      ]}
      break
    default: break
  }
