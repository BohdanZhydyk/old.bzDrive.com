...
export const Info = ({ props:{pri, id, info, name, officeFn} }) => {
  ...
  return(
    <div className={`info${pri}`} key={`Rect${name}`}>

      <div className="bold">{name}</div>

      { inputs.map( (input, index)=> <Input props={{pri, id, input, officeFn}} key={`RectInputs${name}${index}`} /> ) }
