...
const Section5 = ({ props:{pri, line, articles, officeFn} }) => {

  let payDate = pri
    ? `${line.pay.date.day}.${line.pay.date.month}.${line.pay.date.year}`
    : `${line.pay.date.year}-${line.pay.date.month}-${line.pay.date.day}`

  let  PAY_METHOD = (method)=> officeFn({
    type:"PAY_METHOD",
    payload:{id:line._id, method}
  })

  let  PAY_DATE = (date)=> officeFn({
    type:"PAY_DATE",
    payload:{id:line._id, date:inputDateToStandart(date.target.value)}
  })
  ...
  return(
    <div className="section5 flex">
      ...
      <div className="payments">
        ...
        <div className={`amount1${pri} flex bold`}>
